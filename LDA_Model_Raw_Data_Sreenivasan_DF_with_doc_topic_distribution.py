...
def LDA_Model_with_DF_stopwords(DF_threshold):
    ...
    #Tutorial for topic distribution per document: https://github.com/RaRe-Technologies/gensim/blob/develop/docs/notebooks/topic_methods.ipynb
    all_topics = lda_model.get_document_topics(doc_term_matrix, per_word_topics=True)
    topic_distribution_per_doc_file = open("most_likely_topic_per_doc_DF_threshold=" + str(DF_threshold) + ".txt", "w")
    
    case_id = 0
    for doc_topics, word_topics, phi_values in all_topics:
        case_id += 1
        most_likely_topic = max(doc_topics, key = lambda i : i[1])
        #doc_topics = sorted(doc_topics.items(), key=lambda x: x[1], reverse=True)
