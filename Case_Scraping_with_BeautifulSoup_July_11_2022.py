# -*- coding: utf-8 -*-
"""
Created on Mon Jul  4 02:12:03 2022

@author: Acer
"""

import urllib.request, urllib.parse, urllib.error
from bs4 import BeautifulSoup as bs
import ssl
import numpy as np
import re
import csv

# --- ignore ssl certificate ---
ctx = ssl.create_default_context()
ctx.check_hostname = False
ctx.verify_mode = ssl.CERT_NONE
'''
label_row = ['bverfg_id_forward', 'docket_numbers', 'senate_and_chamber', 'participating_judges', 'decision_date', 'full_text']


with open('case_scraping_to_June_2022.csv', mode='w') as case_file:
    case_writer = csv.writer(case_file, delimiter=',',  quotechar='"', quoting=csv.QUOTE_MINIMAL)
    case_writer.writerow(label_row)
    case_file.close()
'''


def parse_case(url, bverfg_id_forward):
#url = 'https://www.bundesverfassungsgericht.de/SharedDocs/Entscheidungen/DE/2020/07/rk20200714_2bvr085220.html'
#url = 'https://www.bundesverfassungsgericht.de/SharedDocs/Entscheidungen/DE/2022/06/qk20220627_1bvq004522.html'
#url = 'https://www.bundesverfassungsgericht.de/SiteGlobals/Forms/Suche/Entscheidungensuche_Formular.html?nn=5399828&facettedYear=2022+2021+2020&language_=de'
    #global bverfg_id_forward #access a global variable inside a function
    bverfg_id_forward = bverfg_id_forward + 1
    print('bverfg_id_forward =', bverfg_id_forward)
    print('url =', url)
    case_row = []
    html = urllib.request.urlopen(url, context=ctx).read() 
    
    soup = bs(html, 'html.parser') 
    #print(soup.prettify())        
    
    date_docket = soup.find('strong', string=re.compile('Beschluss vom|Urteil vom'))
    date_docket = date_docket.text.strip()
    if date_docket[:14] == 'Beschluss vom ':
        date_docket = date_docket[14:] #take out the 'Beschluss vom ' portion
    else: 
        date_docket = date_docket[11:] #alternatively, take out the 'Urteil vom ' portion 
    try:
        date, docket = date_docket.split(' - ')
    except: #in case the docket number is N/A
        date = date_docket
        docket = ''
        
    print('date_docket =', date_docket)
    #print('date =' + date)
    #print('docket =' + docket)
    
    #senate_chamber = soup.find_all('meta')#, content='Bundesverfassungsgericht^') #name = "DC.creator")
    senate_chamber = soup.find('meta', attrs={'name':'DC.creator'})
    senate_chamber = senate_chamber.get('content')
    senate_chamber = senate_chamber.split(',')[1]
    print('senate_chamber =', senate_chamber)
    
    authors = soup.find_all('td', class_='st')
    author_list = []
    for author in authors:
        author = author.text.strip()
        if author[0:3] == 'Dr.':
            author = author[3:]
        if author[0:4] == 'Dr. ':
            author = author[4:]
        author_list.append(author)
    print('author_list =', author_list)
    
    
    decisions = soup.find_all('div', class_='entscheidung')
    decision_str = ''
    for decision in decisions:
        decision = decision.text.strip()
        decision_str = decision_str + decision
    #print('decision_str =', decision_str)
    
    case_row.append(bverfg_id_forward)
    case_row.append(docket)
    case_row.append(senate_chamber)
    case_row.append(author_list)
    case_row.append(date)
    case_row.append(decision_str)
    
    #print('case_row =', case_row)
    return case_row, bverfg_id_forward
    #case_mtrx.append(case_row)
    
    #return date, docket, author_list, decision_str





def process_ten_cases(main_url, idx):
    bverfg_id_forward = (idx-1)*10
    #main_url = 'https://www.bundesverfassungsgericht.de/SiteGlobals/Forms/Suche/Entscheidungensuche_Formular.html?nn=5399828&facettedYear=2022+2021+2020&language_=de'
    main_html = urllib.request.urlopen(main_url, context=ctx).read() 
    main_soup = bs(main_html, 'html.parser')
    case_links = main_soup.find_all('a', href=re.compile('^SharedDocs/Entscheidungen/DE'))
    case_link_list = []
    for case_link in case_links:
        case_link = case_link.get('href')
        case_link = 'https://www.bundesverfassungsgericht.de/' + case_link
        case_link_list.append(case_link)
    print('case_link_list =', case_link_list)
    
    case_mtrx = []

    '''
    def parse_case(url):
    #url = 'https://www.bundesverfassungsgericht.de/SharedDocs/Entscheidungen/DE/2020/07/rk20200714_2bvr085220.html'
    #url = 'https://www.bundesverfassungsgericht.de/SharedDocs/Entscheidungen/DE/2022/06/qk20220627_1bvq004522.html'
    #url = 'https://www.bundesverfassungsgericht.de/SiteGlobals/Forms/Suche/Entscheidungensuche_Formular.html?nn=5399828&facettedYear=2022+2021+2020&language_=de'
        #global bverfg_id_forward #access a global variable inside a function
        bverfg_id_forward = bverfg_id_forward + 1
        print('bverfg_id_forward =', bverfg_id_forward)
        print('url =', url)
        case_row = []
        html = urllib.request.urlopen(url, context=ctx).read() 
        
        soup = bs(html, 'html.parser') 
        #print(soup.prettify())        
        
        date_docket = soup.find('strong', string=re.compile('Beschluss vom|Urteil vom'))
        date_docket = date_docket.text.strip()
        if date_docket[:14] == 'Beschluss vom ':
            date_docket = date_docket[14:] #take out the 'Beschluss vom ' portion
        else: 
            date_docket = date_docket[11:] #alternatively, take out the 'Urteil vom ' portion 
        try:
            date, docket = date_docket.split(' - ')
        except: #in case the docket number is N/A
            date = date_docket
            docket = ''
            
        print('date_docket =', date_docket)
        #print('date =' + date)
        #print('docket =' + docket)
        
        #senate_chamber = soup.find_all('meta')#, content='Bundesverfassungsgericht^') #name = "DC.creator")
        senate_chamber = soup.find('meta', attrs={'name':'DC.creator'})
        senate_chamber = senate_chamber.get('content')
        senate_chamber = senate_chamber.split(',')[1]
        print('senate_chamber =', senate_chamber)
        
        authors = soup.find_all('td', class_='st')
        author_list = []
        for author in authors:
            author = author.text.strip()
            if author[0:3] == 'Dr.':
                author = author[3:]
            if author[0:4] == 'Dr. ':
                author = author[4:]
            author_list.append(author)
        print('author_list =', author_list)
        
        
        decisions = soup.find_all('div', class_='entscheidung')
        decision_str = ''
        for decision in decisions:
            decision = decision.text.strip()
            decision_str = decision_str + decision
        #print('decision_str =', decision_str)
        
        case_row.append(bverfg_id_forward)
        case_row.append(docket)
        case_row.append(senate_chamber)
        case_row.append(author_list)
        case_row.append(date)
        case_row.append(decision_str)
        
        #print('case_row =', case_row)
        
        case_mtrx.append(case_row)
        
        #return date, docket, author_list, decision_str
    '''
    for case_link in case_link_list:
        case_row, bverfg_id_forward = parse_case(url=case_link, bverfg_id_forward=bverfg_id_forward)
        case_mtrx.append(case_row)
    
    #print('case_mtrx =', case_mtrx)
    
    with open('case_scraping_to_June_2022.csv', mode='a') as case_file: #append mode
        #global case_writer
        case_writer = csv.writer(case_file, delimiter=',',  quotechar='"', quoting=csv.QUOTE_MINIMAL)
        case_writer.writerows(case_mtrx)
        case_file.close()

for idx in np.arange(1,840):
    main_url = 'https://www.bundesverfassungsgericht.de/SiteGlobals/Forms/Suche/Entscheidungensuche_Formular.html?gts=5403124_list%253Ddate_dt%252Basc&gtp=5403124_list%253D' + str(idx) + '&language_=de'
    #main_url = 'https://www.bundesverfassungsgericht.de/SiteGlobals/Forms/Suche/Entscheidungensuche_Formular.html?gtp=5403124_list%253D' + str(i) + '&facettedYear=2022+2021+2020&language_=de'
    process_ten_cases(main_url, idx)